...
增量目标检测以类别增量为主，同时支持目标增量。两种模式的训练、验证、早停和调参都只能读取增量数据。禁止旧样本 replay、旧类原始标签和旧数据缓存特征；旧类测试数据只能在增量权重冻结后用于评分。修改增量脚本时必须保持 `configs/incremental_detection_policy.yaml` 与数据来源审计通过。
