...
def build_blackboard(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    state = {
        ...
        "sar_soldier": {
            ...
            "reason": "case-bank specialist slightly improved SAR soldier but reduced lock_all and IR soldier, so unified YOLO11s remains primary.",
        },
...
def render_blackboard_report(state: Dict[str, Any]) -> str:
    ...
    lines = [
        "# Agent Blackboard Report",
        "",
        f"生成时间：{state.get('generated_at')}",
        "",
        "## 总体状态",
        "",
        f"- 当前阻塞项：`{blockers or ['none']}`",
        f"- 数据量：`{dataset.get('image_count')}` images / `{dataset.get('object_count')}` objects",
        f"- sensor 分布：`{dataset.get('sensor')}`",
        f"- scene 分布：`{dataset.get('scene')}`",
        "",
        "## 最终检测器",
        "",
        ...
        "## SAR Soldier",
        "",
        f"- case bank 样本数：`{state.get('sar_soldier', {}).get('case_bank', {}).get('case_count')}`",
        f"- specialist 状态：`{state.get('sar_soldier', {}).get('specialist_status')}`",
        f"- case bank 新鲜度：`{state.get('sar_soldier', {}).get('case_bank_freshness', {}).get('freshness')}`",
        f"- 结论：{state.get('sar_soldier', {}).get('reason')}",
        "",
        "## 提交状态",
        "",
        f"- hidden test ready：`{state.get('submission', {}).get('official_test_ready')}`",
        f"- format confirmed：`{state.get('submission', {}).get('official_format_confirmed')}`",
        f"- format：`{state.get('submission', {}).get('official_format')}`",
    ]
    return "\n".join(lines) + "\n"
