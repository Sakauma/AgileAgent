...
def build_operator_snapshot(state: Dict[str, Any], decision: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "detector": {
            ...
            "map50": state.get("detector", {}).get("combined_all_map50", state.get("detector", {}).get("lock_all_map50")),
            "sar_map50": state.get("detector", {}).get("combined_sar_map50", state.get("detector", {}).get("lock_sar_map50")),
...
def render_console(snapshot: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "-" * 72,
        f"基础检测    {detector.get('name')}    mAP50={detector.get('map50')}    SAR={detector.get('sar_map50')}",
