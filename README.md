...
当前 production 固定为通过全量750张模拟3+1验收的“双检测器增量代际”。冻结三类基础检测器永久负责旧类别，增量专家从通用预训练表征初始化、只读取新增类 train/dev 并只负责新增类别；两个 owner 对每张未知图共同推理后做框级融合，不使用场景到目标类别的硬路由。候选必须先通过前三项官方精度指标与完整性门禁（`competition_accepted`），以及新增类 precision 和老图误激活部署门禁（`deployment_accepted`），才能进入 production；第四项端到端 FPS 在 Ascend 310B 上独立计分。
...
| 舰船 3+1 类别增量 | 当前750张模拟精度三项为 `50/50` 满分档且通过部署门禁 | 基础 mAP50 `0.81414` / New-mAP50 `0.63869` / KRR `1.00000` / 新类 precision `0.92453` / 老图误激活 `1/70`；未包含板端 FPS 的10分，且不代表官方隐藏测试成绩 |
| 后续官方增量数据 | 模板就绪 | 替换类别与清单后重新训练，增量阶段仍只读取当轮新增类数据 |
| Ascend 310B | 板端推理可用，尚未达到 FPS 满分档 | 据本轮 SSH 只读复核，正式 release 健康状态为 `ready`；89图平均引擎 `57.849 ms` / 墙钟 `71.491 ms`，该批次不是 HTTP 压测。AIPP staging 候选的真实 multipart PNG API 为平均 `51.203 ms` / P95 `63.9 ms` / `19.53 FPS`，按官方分档仅落入 `FPS ≥ 10` 的4分档。已解码核心候选虽为 `31.11 FPS`，但不具备端到端计分资格 |
...
性能指标总分为60分，由四个独立计分项组成。赛题原文记作 `mAP`；本仓库当前评分器将检测指标实现为 `mAP@0.5`（下文记作 `mAP50`），最终提交前仍须以官方评测程序对 IoU 口径的解释为准。

| 计分项 | 分值 | 正确计算口径 | 完整评分档位 |
|---|---:|---|---|
| 基础目标检测 mAP | 30 | 在不含增量类别的基础测试集上评估基础类别；仓库字段为 `base_test_map50` | `≥0.80: 30`；`≥0.70: 25`；`≥0.65: 20`；`≥0.60: 15`；`≥0.50: 10`；`≥0.40: 5`；`<0.40: 0` |
| New-mAP | 10 | 增量后在包含全部已学类别的测试集上，只对本轮新增类别计算 mAP；仓库字段为 `new_map50` | `≥0.60: 10`；`≥0.50: 7`；`≥0.40: 4`；`<0.40: 0` |
| KRR | 10 | 在同一综合测试集上计算 `old_map_after / old_map_before` | `≥0.95: 10`；`≥0.90: 7`；`≥0.80: 4`；`<0.80: 0` |
| Ascend 310B 端到端 FPS | 10 | 单图 `batch=1`，完整处理一帧多模态数据，包含输入处理、三个模型、后处理与结果生成 | `≥30: 10`；`≥20: 7`；`≥10: 4`；`<10: 0` |

前三项精度合计50分，第四项端侧效率10分。`30 FPS` 是效率满分线，不是“低于即不得分”的最低线；例如当前 AIPP staging 真实 API 的 `19.53 FPS` 低于20，按分档只能计4分，而不是7分。仓库内部使用的 `≥36 FPS` 与 P95 `≤33.33 ms` 是留余量的工程发布目标，不改变官方评分。

`0.80` 是赛题基础检测满分线，不是训练提前停止条件。基础训练必须跑满160 epoch、增量训练必须跑满80 epoch，最终 `best.pt` 只在完整预算结束后按各自 dev mAP50 选择。

四类总体 `full_map50` 只作为诊断指标，**不是**“New-mAP ≥ 0.60”的替代项，也不得参与前三项精度计分。`base_dev` 只允许用于 checkpoint 选择，不得冒充基础测试成绩。
...
以上三条数据准备与配置检查命令均通过后，才运行依赖真实数据的命令：
...
生产配置会在缺少显式 lock 时按固定种子和类别组合自动封存20%样本；封存内容不进入训练 YAML。训练完成后，增量学习守护器自动完成 dev 诊断、动态混淆图、逐类阈值校准、代际注册、独立 lock 复核和 shadow 预热。当前750张图严格3+1模拟负责基础 mAP50、New-mAP50 和 KRR 三项精度计分；数据隔离与资产一致性仍是发布完整性前提。四类总体 mAP50 和 x86 时延只生成诊断；新增类 precision 与老图误激活率不改变官方分数，但属于 production 必过质量门禁。第四项端到端 FPS 必须在310B板端按完整单帧链路独立计分。
...
历史混合 FP16 CUDA 代理的基础 mAP50 `0.81954`、New-mAP50 `0.63869`、KRR `1.00000`、新类 precision `0.92453`、老图误激活率 `0.01429`，前三项官方精度满分线与两项内部质量门禁均通过；但严格输出一致候选为 `26.97 FPS`，未优于同配置 FP32 的 `27.63 FPS`。启用新增类最低阈值前移后虽可达 `31.81 FPS`，却改变 `1/89` 张最终结果，因此已拒绝。开发板现已完成独立 ATC/OM 和板端验收，当前结论以 [Ascend 310B 当前工程评估](docs/ascend-310b-current-status.md) 为准；CUDA 代理的 FPS 不计入310B官方端到端效率分。
...
- Ascend 310B 已完成 OM、PyACL/AscendCL 集成和真实板端测试，但正式 release 的89图记录不是官方端到端压测；AIPP staging 候选的真实 PNG API 为平均 `19.53 FPS`，只处于官方4分档，尚未达到 `30 FPS` 满分线。一小时稳定性和 DVPP 完整89张精度门禁也尚未完成。
