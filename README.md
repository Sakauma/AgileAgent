...
用户无需选择模型。Agent 先用图像内容 SHA256 查询冻结的数据血缘：原始域只执行三类基础检测器；已验收的增量域执行基础检测器与当前活动代际专家；未登记输入默认走基础模型并在决策轨迹中标记来源未知。之后系统完成全局 ID 映射和 class-aware NMS。未通过验收的候选模型不会进入默认检测链路。
...
批次通过门禁并晋升后，Agent 会冻结该批次图像指纹并登记为增量域。后续检测按数据来源选择推理链路，不会让增量专家扫描已登记的原始域图像。增量域允许同时出现旧类别和新增类别：旧类别继续由冻结基础模型负责，新增类别由当前代际专家负责。
...
历史组合系统在 95 张内部 lock-val 上取得 mAP50 `0.81929`、舰船 precision `1.000` 和 recall `0.79012`。来源感知路由启用后，原始域的专家执行率固定为 `0`；增量域误激活率必须由不含新增类别的增量负样本另行复核，不能用原始域样本替代。检测指标均指 mAP50；这些结果不是官方隐藏测试成绩。
...
当前代码基线为 `202 passed`。修改模型、推理后端或依赖后，必须重新执行三项验收。
