...
Web 前端面向评委和检测使用者，提供单图检测、批量检测、自动 IR/SAR 与场景识别、标注图预览以及 JSON/ZIP 导出。单图页可切换“标准识别”和“增量演示”：增量模式由场景认知模型判断当前场景，Agent策略决定是否激活已通过门禁的新类specialist，再与冻结旧类检测器组合输出。单GPU采用公平队列串行推理；单文件限制20MB、单批限制20张和200MB，当前会话最多保留20条轻量历史。权重路径、哈希和部署门禁等运维细节不会显示在 Web 中。
