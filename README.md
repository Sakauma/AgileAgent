...
Web 前端面向检测使用者，提供单图检测、批量检测、自动 IR/SAR 与场景识别、标注图预览以及 JSON/ZIP 导出。单GPU采用公平队列串行推理；任务按图像内容SHA256标识，同名文件不会复用旧结果。单文件限制20MB、单批限制20张和200MB，当前会话最多保留20条轻量历史。模型指标、哈希、增量协议、审计和部署门禁不会显示在 Web 中。
