...
新时序划分的严格候选 `strict-yolo11m-generic-20260806-215608` 已在服务器完整跑满基础160 epoch和增量专家80 epoch。冻结权重及推理设置后，两个 owner 先对全部89张混合测试图执行无标签推理，再解封标签评分：70张纯旧类基础测试图的 mAP50 为 `0.80013`，完整混合测试的 New-mAP50 为 `0.76301`，KRR 为 `1.00000`，三项均达到满分档门槛。该结果是750张现有数据的本地代理测试，不保证官方后续全新增量数据和隐藏测试成绩，也没有自动覆盖既有 production。
