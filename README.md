...
|---|---:|---|---|
| 五折调参折（289张） | `0.87642` | 选择固定逐类融合规则 | 官方/独立测试声明 |
| 未参与选择的后置折（186张） | `0.88872` | 检查所选规则的开发泛化 | 官方/独立测试声明 |
| 完整连续块 OOF（475张） | `0.88063` | 当前最强非测试开发证据 | 替代隐藏测试成绩 |
| 48张 checkpoint-val sanity | `0.94588` | 在完整160 epoch预算内选择 `best.pt`、检查 refit 一致性 | 性能证据；该集合参与过 checkpoint 选择 |
| 前向块3调参（93张） | 基线 `0.84604`；候选 `0.85761` | 冻结时序候选 | 证明候选能外推到下一时刻 |
| 前向块4后置回归（93张） | 基线 `0.88162`；候选 `0.87344` | 拒绝不稳健候选 | 再次充当独立验证或继续选模 |
...
下一层稳健开发协议由 `tools/92_build_base_rolling_forward.py`、`tools/93_select_base_class_expert.py` 和 `tools/94_validate_base_class_expert.py` 固定：候选只在多个更早的滚动起点上按最差起点 AP50 和逐起点退化门禁选择，已解封的最后一块不得参与选择。困难类 owner 在候选训练前依据基础开发集逐类 AP 确定，不绑定 soldier 等语义名称；当前数据的瓶颈恰好是基础局部类0。该 owner 只负责一个既有基础类别，但部署时仍对每张未知图片无条件运行，禁止任何图片级类别判断或场景硬路由。最后一块即使回归超过0.85，也仍不是新的独立测试证据。
...
新时序划分最早的严格候选 `strict-yolo11m-generic-20260806-215608` 已在服务器完整跑满基础160 epoch和增量专家80 epoch。其早期复核为基础 `0.80013`、New-mAP50 `0.76301`、KRR `1.00000`；随后首次冻结基础 ensemble 的独立代理结果约为基础 `0.80147`、New-mAP50 `0.81998`、KRR `1.00000`。当前 `base-s896-classwise-oof-20260807-v5` 的开发 OOF 已提升至 `0.88063`，但89张锁集已经解封，后续回归不能升级为新的独立证据。该结论不保证官方后续全新增量数据和隐藏测试成绩，也没有自动覆盖既有 production。
