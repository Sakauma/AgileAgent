...
2026-08-26 实机整链结果为 Base mAP50 `0.816663`、New-mAP50 `0.624935`、KRR `1.000000`、Full-mAP50 `0.726497`。当时的性能报告未包含正式结果落盘，现已归档；新版门禁会在写出正式六列 TXT 后按总帧数除以总墙钟耗时计算 aggregate FPS。通过门禁后，`demo_report.json` 给出本次隔离配置；使用它启动 CLI 即可查看学习后的结果：
