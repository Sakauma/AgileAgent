...
本文记录截至 2026-08-14 的 Ascend 310B 工程状态，并严格区分仓库实现、正式板端只读复核、已批准精度、不同测量边界的性能，以及尚未进入正式配置的候选方案。结论是：板端三模型推理和精度门禁已经跑通，可用于功能演示与继续优化；端到端 30 FPS、长期稳定性和可复现发布证据仍未闭环，不能签署为最终性能验收通过。
...
| 正式 release 精度 | Base mAP50 `0.819407`、New mAP50 `0.728761`、KRR `1.0`、新类 precision `0.933333`、误激活率 `0.014286`，五项门禁通过。 |
| AIPP staging 候选精度 | Base mAP50 `0.819415`、New mAP50 `0.728761`、KRR `1.0`、新类 precision `0.933333`、误激活率 `0.014286`；通过精度门禁，但没有切换正式 release。 |
...
| 真实 PNG API 候选 | AIPP staging 服务的 1,068 次 multipart PNG 请求：服务端均值 `51.203 ms`、P95 `63.9 ms`、`19.53 FPS`。即使候选也未达到端到端 30 FPS。 |
...
| 310B 精度 | 当前正式 OM 通过 | 正式 release 五项门禁通过；任何 OM、AIPP、DVPP 或 ATC 变更都需独立重跑。 |
| 310B 性能 | 未通过 | 正式89图和 staging API 证据均未达到端到端30 FPS；只有不含 multipart/PNG 的核心候选超过30 FPS。 |
...
两套结果都通过五项门禁，但只有左列属于当前正式 release；右列用于说明 AIPP 候选未破坏总体验收指标，不能据此声称 production 已切换。两者也不应与 [`models/generations.json`](../models/generations.json) 中较早的仓库基线混写为同一次评测。

## 性能边界

| 测量对象 | 样本量 | 均值 | P95 | FPS | 判定 |
|---|---:|---:|---:|---:|---|
| 正式 release 完整89图 | 89 | 引擎 `57.849 ms`；墙钟 `71.491 ms` | 未记录 | 引擎约 `17.29`；墙钟约 `13.99` | 正式链路可运行，性能未达标；该批次不是 HTTP 压测。 |
| 已解码 Agent 核心 | 200 | `32.148 ms` | `33.193 ms` | `31.11` | 仅核心路径超过 30 FPS，不代表 API 端到端。 |
| DVPP 编码输入候选 | 240 | `37.124 ms` | `38.154 ms` | `26.94` | 实验结果，默认关闭。 |
| AIPP staging 真实 multipart PNG API | 1,068 | `51.203 ms` | `63.9 ms` | `19.53` | 已验证的候选端到端结果，未达到 30 FPS；正式服务未切换到该候选。 |
...
- 当前正式 release 的完整89图平均引擎耗时为 `57.849 ms/图`；性能候选的真实 multipart PNG API 也只有 `19.53 FPS`，端到端 30 FPS 仍是未关闭目标。
...
4. 以真实 API 的 P95 和 FPS 为优化目标，定位 multipart、PNG 处理、排队和响应生成开销；不得用已解码核心 `31.11 FPS` 替代端到端结果。
