...
```text
  识别    [1] 单图识别    [2] 批量识别
  查看    [3] 最近结果    [4] 运行状态    [5] 模型信息
  系统    [r] 刷新状态    [h] 使用帮助    [q] 退出
```

交互式终端默认启用圆角卡片、状态色和高亮快捷键；重定向输出、`TERM=dumb` 或设置了 `NO_COLOR` 时会自动切换为无颜色文本，内容和操作方式不变。如需在支持 ANSI 的非交互环境强制启用颜色，可设置 `FORCE_COLOR=1`。

- `1`：粘贴一张图像路径并执行正式识别。
- `2`：输入图像目录，可选择是否递归扫描子目录。
- `3`：查看最近八次识别的时间、图像数、目标数和保存目录。
- `4`：查看架构、设备、推理后端、配置来源与影响本机识别的问题。
...
- 单图识别显示统计摘要、图像上下文和最终检测目标明细。
- 批量识别只显示图像数、有目标/未检出图像数、目标数、全流程总耗时与 FPS、类别分布和上下文分布，不在终端逐图展开检测框。
- 批量任务最多输出约十次匿名进度，不打印每张图像的文件名。完整逐图结果始终保存在 `results.json`、`detections.csv` 和 `predictions/`；需要机器可读的完整标准输出时可使用 `--format json`。
...
Ascend310B 批量识别默认以 20 图为一个请求，并复用正式服务的三实例推理池。终端以“全流程总耗时 / FPS”作为门禁口径，按总图像数除以从输入读取开始到结果写出完成的总墙钟耗时计算；包含 HTTP 处理、图像解码、Scene、决策、Base/Incremental 检测、后处理和结果落盘。`results.json` 仍保留 engine/service 分项耗时供诊断，但这些分项不再冒充官方 FPS。
...
CLI 会自动选择复用已启动的正式服务或在当前进程加载识别引擎。首次加载模型可能需要一些时间，后续识别通常会更快。在 310B 正式部署中如长时间没有响应，可检查：
