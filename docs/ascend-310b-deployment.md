...
> 状态：三模型 OM、PyACL/AscendCL 后端、89图精度与真实 PNG 推理均已跑通；端到端FPS尚未达到30 FPS的10分满分线，1小时稳定性也尚未通过
> 状态日期：2026-08-14
...
性能指标总分60分，由四个独立计分项组成。赛题原文使用 `mAP`；仓库当前精度工具实现为 `mAP@0.5`（`mAP50`），最终仍须以官方评分程序的IoU口径为准。
...
| Ascend 310B端到端FPS | 10 | `≥30:10`；`≥20:7`；`≥10:4`；`<10:0` |

前三项精度合计50分，第四项端侧效率10分。官方FPS必须测量单图 `batch=1` 完整处理一帧多模态数据的端到端帧率；只报告OM、Agent核心、已解码输入或批量吞吐均不能代替。`30 FPS` 是10分满分线，不是低于即得0分的最低线。仓库额外使用 precision、误激活率、P95、稳定性和回滚作为发布门禁，这些不增加官方分数。
...
4. 正式 release 使用 CPU 解码/预处理；AIPP staging 候选已通过前三项官方精度和两项内部质量门禁，DVPP/VPC 仍只通过12图 preflight，默认关闭。
...
官方端侧效率的10分满分线是30 FPS，20 FPS和10 FPS分别对应7分与4分。本方案把内部工程目标固定为端到端平均吞吐不低于36 FPS、P95不高于33.33 ms，以给温度和抖动留出余量。当前 AIPP staging 的真实 multipart PNG API 为平均 `51.203 ms`、P95 `63.9 ms`、`19.53 FPS`，严格落入4分档；只有已解码 Agent 核心达到 `31.11 FPS`，不能据此领取端到端FPS分数。
...
正式 OM 的 KRR 为 `1.000000`，前三项官方精度自测为 `50/50`。内部部署门禁还包括新类 precision `0.933333` 和70张旧类图上的误激活率 `0.014286`；这两项不增加官方分数。以上结果来自当前750张图的固定模拟划分，不代表官方隐藏测试成绩。AIPP staging 候选对应基础 mAP50 为 `0.819415`，其余精度/质量项相同；该候选没有切换正式 release。

正式板端结果相对满分线的余量分别为：基础 mAP50 约 `0.0194`、New-mAP50 约 `0.1288`。任何候选仍必须先通过前三项官方精度满分线、两项内部质量门禁和无泄露检查，再比较真实性能；不能用平均指标余量容忍逐图检测数变化。
...
| --- | ---: |
| 输入模式 | 单图、`batch=1` |
| 官方效率评分 | `≥30 FPS:10分`；`≥20:7分`；`≥10:4分`；`<10:0分` |
| 内部端到端平均吞吐 | `≥36 FPS` |
| 端到端 P95 | `≤33.33 ms` |
...
先完成固定形状混合 FP16 的端到端迁移。前三项是官方精度计分满分线，后两项是仓库内部发布质量门禁：
...
建议同时记录相对 PyTorch 基线的总体 mAP50 差值与逐类 AP 差值；任何指标即使只下降少量，只要跨过上述精度/质量硬门槛就必须拒绝上线。第四项官方FPS在G5端到端协议中单独计分。
...
前三项官方精度满分线和两项内部质量硬门槛全部通过；相对同次 PyTorch FP32 的最大指标绝对差为 `0.00544`，来自基础 mAP50 的小幅上浮，因此没有通过工具中更严格的“近似逐指标等价”阈值。原始张量余弦相似度最低值分别为基础检测器 `0.99999897`、增量检测器 `0.99999877`、Scene-SensorNet `0.99953009`；Scene 对 FP16 更敏感，但当前门禁未改变。另测“两个检测器 FP16、Scene 保持 FP32”，上述三项精度与两项内部质量指标和全 FP16 方案相同。
...
最大指标绝对差为 `9.8×10⁻⁵`，前三项官方精度满分线和两项内部质量门禁全部通过。golden bundle 位于 `runs/ascend310b/golden/rect/`，包含3张无标签代表图、三路输入张量和 ONNX 原始输出，供板端逐元素对齐；该目录被 Git 忽略。
...
- [x] 本机混合 FP16 ONNX 代理通过前三项官方精度满分线和两项内部质量门禁；未将 CUDA 代理 FPS 误写为310B端到端证据。
...
- [ ] DVPP/VPC 仅完成12图 preflight；AIPP 候选完成89图三项官方精度和两项内部质量门禁，但两者均未切换正式 release。
...
- [x] PyTorch/ONNX 的三项官方精度满分线与两项内部部署质量门禁全部通过。
- [x] 板前无标签全量推理和无泄露审计通过。
- [x] OM 完整 Agent 的三项官方精度满分线、两项内部质量门禁与无泄露审计通过。
- [ ] 1 小时稳定性、温度和回滚测试通过。
...
如果不存在上述路径，常见替代位置是目标机外部路径 /usr/local/Ascend/ascend-toolkit/set_env.sh，或 Runtime 安装目录中的 set_env.sh。必须以已安装版本为准，不要通过创建伪造的 `latest` 软链接掩盖版本差异。
...
当前 DVPP 候选在12张图上保持了 `12/12` 的检测数量、类别序列和 context 标签，但因 VPC 插值造成概率/坐标变化，`0/12` 达到最终结果逐元素完全一致；完整89图的三项官方精度与两项内部质量门禁尚未执行。其240样本性能为平均 `37.124 ms`、P95 `38.154 ms`、`26.94 FPS`，但该测量不是完整 API，不能据此领取官方7分；因此继续保持 `encoded_preprocessing: cpu`，不得上线 DVPP。
...
golden 通过并不等于完整精度通过；它只用于快速确定实现层级。最终仍以完整混合集的三项官方精度和两项内部质量指标为准。
...
### 27.4 前三项官方精度与两项内部质量门禁

| 指标 | 性质 | 评分范围 | 满分线/门槛 | 正式 release | AIPP staging 候选 |
| --- | --- | --- | ---: | ---: | ---: |
| Base-mAP50 | 官方，最高30分 | 70张纯旧类基础测试，旧类 `0/1/3` | `≥0.80` | `0.819407`（30分档） | `0.819415`（30分档） |
| New-mAP50 | 官方，最高10分 | 完整89张混合集，只评新类 `2` | `≥0.60` | `0.728761`（10分档） | `0.728761`（10分档） |
| KRR | 官方，最高10分 | 完整89张混合集，旧类 after/before | `≥0.95` | `1.000000`（10分档） | `1.000000`（10分档） |
| 新类 precision | 内部质量门禁，不计分 | 完整混合集，冻结阈值 | `≥0.90` | `0.933333` | `0.933333` |
| 旧类图新增误激活率 | 内部质量门禁，不计分 | 70张旧类图 | `≤0.05` | `0.014286` | `0.014286` |

正式 release 和AIPP候选的前三项精度自测均为 `50/50`。AIPP候选通过上述五项精度/质量门禁只代表精度与内部质量可接受；第四项官方端到端FPS必须在G5单独计分。AIPP staging真实API为 `19.53 FPS`，严格落入4分档，因此现有候选的四项自测估算是 `54/60`，不是production或官方成绩。

`full_map50` 只报告诊断，不能替代 New-mAP。KRR 为 1 也不能替代 Base-mAP50；新类误框不会自动降低旧类 KRR，因此 precision 和误激活门禁必须独立检查。

候选保留规则：前三项官方精度与两项内部质量门禁全部通过、无泄露审计通过、每图 owner 语义正确。任一失败都回到 G2/G3 定位，不能在 mixed test 标签上搜索新阈值；随后才进入第四项FPS测试。
...
| --- | ---: | ---: | ---: | ---: | --- |
| 正式 release 完整89图 | 89 | 引擎 `57.849 ms`；墙钟 `71.491 ms` | 未记录 | 引擎约 `17.29`；墙钟约 `13.99` | 非HTTP/官方压测；若该边界被接受则为4分档 |
| AIPP staging 已解码 Agent 核心 | 200 | `32.148 ms` | `33.193 ms` | `31.11` | 非端到端，不能据此领取FPS 10分 |
| DVPP staging 候选 | 240 | `37.124 ms` | `38.154 ms` | `26.94` | 非完整API且默认关闭，不能据此领取FPS 7分 |
| AIPP staging 真实 multipart PNG API | 1068 | `51.203 ms` | `63.9 ms` | `19.53` | 当前最完整的候选API证据，落入4分档 |
...
| 项目 | 分档/通过线 |
| --- | ---: |
| 官方端到端FPS | `≥30:10分`；`≥20:7分`；`≥10:4分`；`<10:0分` |
| 内部平均吞吐 | `≥36 FPS` |
| 端到端 P95 | `≤33.33 ms` |
| 请求错误 | 0 |
| 输出正确性 | G4前三项官方精度、两项内部质量门禁与合规门全部通过 |
...
| 顺序 | 优化 | 主要目标 | 验证要求 |
| ---: | --- | --- | --- |
| 0 | 正确的串行 ACL 基线 | 建立可解释结果 | golden + 三项官方精度与两项内部质量门禁 |
...
| 6 | ATC `mixed_float16` | 降模型延迟/内存 | 三项官方精度与两项内部质量门禁全部通过 |
...
2. 若三模型、三项官方精度和两项内部质量门禁通过，进入第四项端到端FPS测试。
...
通过条件：零请求错误、三个 owner 计数相等、没有持续内存增长、没有设备复位或长期降频、最后 10 分钟性能未明显劣于最初稳态窗口。完成后再次跑 golden、三项官方精度和两项内部质量指标，防止长时间运行或热切换改变状态。
...
- [x] Base-mAP50、New-mAP50、KRR 三项官方精度满分线，以及 precision、误激活率两项内部质量门禁通过。
...
2. 冻结后再开放标签评分。
3. 三项官方精度满分线、两项内部质量门禁与无泄露检查全部通过。
