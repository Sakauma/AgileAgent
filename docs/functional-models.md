...
| Incremental Model Bank | 仅使用增量数据的模式感知增量目标检测 | 增量图像/标签、冻结基础权重、增量模式 | 组合预测、路由轨迹、New-mAP50、KRR | p02-p04 目标增量演练通过；strict-p02 舰船类别增量通过 |
...
Scene-SensorNet 不改变 YOLO 的输入张量；其概率仅作为软路由排序依据，不执行场景硬拒绝。目标增量模型需要基础同类目标和空间一致性证据；类别增量模型使用基础类别集合外的 ID，并通过独立校准阈值激活。两种模式的训练和验证都只能读取增量数据。现有 p01-p04 目标类别已包含在四类基础模型中，因此仅作为专项增强演练。strict-p02 则使用不含舰船输出通道的三类基础模型，舰船 specialist 不依赖基础同类框，已通过 New-mAP50 0.90903、KRR 1.00000 和数据合规门禁。
...
三种功能均已通过 x86 NVIDIA GPU 加载和推理验收。Incremental Model Bank 仍标记为 `partially_verified`：目标增量 p01 和严格类别增量 strict-p01 未通过，但 strict-p02 已作为独立实验档注册。strict-p02 的 lock 图像误激活率为 0.33784，暂不替换默认 Web 模型。三个模型均未完成 Ascend 310B 转换，因此注册表明确保留 `ascend_310b: false`，不得把 x86 smoke test 当作板端证据。
