...
| Incremental Model Bank | 仅使用增量数据的模式感知增量目标检测 | 增量图像/标签、冻结基础权重、增量模式 | 组合预测、路由轨迹、New-mAP50、KRR | strict-p02 核心指标通过，部署门禁待通过 |
...
Scene-SensorNet 不改变 YOLO 的输入张量；其概率仅作为软路由排序依据，不执行场景硬拒绝。目标增量模型需要基础同类目标和空间一致性证据；类别增量模型使用基础类别集合外的 ID，并通过独立校准阈值激活。两种模式的训练和验证都只能读取增量数据。现有 p01-p04 目标类别已包含在四类基础模型中，因此仅作为专项增强演练。strict-p02 使用不含舰船输出通道的三类基础模型，舰船 specialist 不依赖基础同类框；New-mAP50 0.90903、KRR 1.00000 和数据合规门禁通过，但部署 precision 与误激活门禁未通过。
...
三种功能均已通过 x86 NVIDIA GPU 加载和推理验收。Incremental Model Bank 整体仍标记为 `partially_verified`，因为目标增量 p01 和严格类别增量 strict-p01 未通过；这不影响独立验收的舰船类别增量能力。历史 strict-p02 阈值 0.51 的误激活率为 0.33784，未上线；`generation-1-recheck-v2` 使用增量 dev 固定的 0.63 阈值，在 TensorRT lock-val 复核中取得 New-mAP50 0.795、KRR 1.0、precision 1.0、误激活率 0.0，已进入默认 Web production。三个模型均未完成 Ascend 310B 转换，因此注册表明确保留 `ascend_310b: false`，不得把 x86 smoke test 当作板端证据。
