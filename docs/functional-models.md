...
| Incremental Model Bank | 无旧类原始数据的新类快速学习 | 新类图像、冻结教师输出、关注类别 | 增量预测、New-mAP50、KRR、门禁结论 | p02-p04 通过，p01 待改进 |
...
Scene-SensorNet 不改变 YOLO 的输入张量；其预测进入 Agent 策略上下文。增量模型也不会自动覆盖统一检测结果，只有通过 New-mAP50 与 KRR 门禁的协议才会产生人工复核候选。注册表中的 `status` 字段明确区分已实现链路与门禁链路。
