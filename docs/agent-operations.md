...
上传图像和结果只保留在当前 Streamlit 会话内，不写入训练数据或固定报告目录。Web 不显示权重哈希、模型指标、增量协议、门禁和审计细节；这些内容统一由 CLI 前端管理。
...
该命令依次执行环境诊断、重建黑板并进入面向开发与运维人员的交互式 CLI 前端。它不会启动 Streamlit，也不会安装或修改依赖。
