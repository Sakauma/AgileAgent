...
- 自动能力编排：Agent 先根据图像内容指纹查询冻结的数据血缘。原始域只执行基础检测器；已验收的增量域执行基础检测器与当前活动代际专家；未知来源默认只执行基础检测器。场景认知只提供上下文，不作为固定激活限制。
...
界面的“纯推理时间”是本次实际参与模型各自 GPU 前向时间之和，包括上下文模型、基础检测器，以及仅在增量域启用的类别专家；不包含上传、排队、图像预处理、框后处理、绘图和结果编码。模型并行时该数值表示计算量之和，不等同于关键路径时延。“系统总用时”从服务收到请求开始，统计到结果 JSON 构建完成，包含完整处理链路，是判断单请求速度的口径。
...
- 来源路由同样使用内容SHA256，不依赖容易变化的文件名、场景预测或用户手工选择。
