...
def test_packaged_release_config_and_manifests_are_self_contained() -> None:
    ...
    detector = next(iter(ascend["models"].values()))

    assert config["runtime"]["server_port"] == 18501
    assert ascend["validated"] is True
    assert ascend["validation_candidate"] is False
    assert ascend["model_layout"] == "shared_backbone_dual_head_v1"
    assert ascend["context_mode"] == "fixed_neutral_v1"
...
def test_model_manifest_promotes_packaged_ascend_release() -> None:
    ...
    release = manifest["ascend_releases"][0]

    assert release["id"] == "20260816-full-score-1493b04"
    ...
    assert min(release["metrics"]["post_promotion_batch_fps"]) >= 30.0
...
def test_materializer_is_zero_training_and_preserves_ports() -> None:
    ...
    assert 'RELEASE_ROOT="${RELEASE_PARENT}/20260816-full-score-1493b04"' in script
    assert "sha256sum -c SHA256SUMS" in script
    assert "tools/95_verify_ascend_release.py" in script
    assert "tools/107_train_shared_dual_head.py" not in script
    assert "build_ascend_dual_head_om.sh" not in script
...
def test_ascend_start_and_stop_default_to_packaged_production_release() -> None:
    expected = "/home/HwHiAiUser/agileagent/releases/20260816-full-score-1493b04"
