...
def write_readme(
    ...
    text = [
        "# Submission Prediction Package",
        "",
        "Generated by `tools/42_predict_submission.py`.",
        "",
        f"- model: `{config['model']}`",
        f"- imgsz: `{config['predict']['imgsz']}`",
        f"- conf: `{config['predict']['conf']}`",
        f"- iou: `{config['predict']['iou']}`",
        f"- max_det: `{config['predict']['max_det']}`",
        f"- min_export_conf: `{format_optional(min_export_conf)}`",
        f"- images: `{image_count}`",
        f"- predictions: `{prediction_count}`",
        f"- raw_predictions: `{raw_prediction_count}`",
        "",
        "## Files",
        "",
        "- `labels/`: one YOLO txt per image, format `class x_center y_center width height`.",
        "- `labels_conf/`: one YOLO txt per image, format `class x_center y_center width height confidence`.",
        "- `predictions.csv`: flat prediction table with normalized and absolute boxes.",
        "- `predictions.json`: same predictions as structured JSON.",
        "- `manifest.json`: run metadata and output hashes.",
        "",
        "Use the format required by the official evaluator. Keep the CSV/JSON files for conversion if the required format changes.",
    ]
    path.write_text("\n".join(text) + "\n", encoding="utf-8")
