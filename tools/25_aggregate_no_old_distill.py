...
def main() -> int:
    ...
    for protocol in config["protocols"]:
        ...
        row.update({
            ...
            "incremental_mode": scope["incremental_mode"],
        ...
        rows.append(row)
    fields = ["protocol", "task_type", "incremental_mode", "learning_data_scope", "learning_scope_verified", "method", "new_classes", "old_map50_before", "old_map50_after", "new_map50_after", "full_map50_after", "krr", "training_seconds", "training_image_count", "old_raw_image_count", "validation_old_raw_image_count", "frozen_parameter_max_abs_drift", "passed"]
    flat_rows = []
    for row in rows:
    ...
    lines = [
        "# 合规的无旧数据增量学习汇总",
        "",
        "任务类型为增量目标检测，当前协议模式为类别增量，并保留目标增量接口。训练、验证、早停和调参仅使用增量数据；旧类测试数据只在权重冻结后的评分阶段读取。",
        "",
