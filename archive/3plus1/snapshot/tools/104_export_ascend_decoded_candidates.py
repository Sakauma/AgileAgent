...
def _probe(args: argparse.Namespace) -> dict:
    ...
    module = build_decoded_candidates_v1_module(
        ...
        candidate_capacity=int(args.candidate_capacity),
    )
    sample = torch.zeros(
        (1, 4 + int(args.classes), int(args.anchors)),
        dtype=torch.float32,
    )
    sample[:, 2:4] = 2.0
    with torch.inference_mode():
        outputs = module(sample)
    exported = export_decoded_candidates_v1_onnx(
        module,
        sample,
    ...
    return {
        "schema_version": 1,
        "kind": "ascend_decoded_candidates_v1_probe",
        "input_shape": list(sample.shape),
        "output_shapes": model_output_shapes(outputs),
        "candidate_confidence": float(args.candidate_confidence),
        "candidate_capacity": int(args.candidate_capacity),
        "class_count": int(args.classes),
        "anchor_count": int(args.anchors),
        "onnx": exported,
    }
...
def _export(args: argparse.Namespace) -> dict:
    ...
    for spec in _detector_specs():
        ...
        module = build_decoded_candidates_v1_module(
            ...
            candidate_confidence=float(args.candidate_confidence),
            candidate_capacity=int(spec["candidate_capacity"]),
        ).to(device)
        sample = torch.zeros(spec["input_shape"], device=device)
        with torch.inference_mode():
            outputs = module(sample)
        anchor_count = int(outputs[-1].shape[2])
        exported = export_decoded_candidates_v1_onnx(
            module,
            sample,
    ...
    payload = {
        "schema_version": 1,
        "kind": "ascend_decoded_candidates_v1",
        "candidate_confidence": float(args.candidate_confidence),
        "opset": int(args.opset),
        "device": str(device),
        "assets": rows,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "导出P9 decoded_candidates_v1固定输出ONNX：设备只做严格"
            "score>0.01解码/收集，Host保留精确排序与NMS。"
        )
    )
    ...
    probe.add_argument("--candidate-capacity", type=int, default=32)
